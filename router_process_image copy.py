...
# Hapus import OpenAI dan inisialisasi client OpenAI
...
def get_label_from_gpt(image_bytes: bytes) -> str:
    try:
        ...
        prompt = (
            "This is an image. "
            "Please reply ONLY with ONE WORD (in English) that best represents the main object in the image. "
            "Do not give any explanation. Do not add any extra words. Just reply with one word only."
        )

        logger.debug("Sending request to Together AI Vision Model.")
        ...
        logger.info(f"Raw label response: {label_raw}")
        # Ambil hanya satu kata pertama
        label = label_raw.split()[0]
        return label
    except Exception as e:
        logger.error(f"Failed to get label from TogetherAI: {e}")
        return "unknown"
...
async def overwrite_label(data: UpdateLabelRequest):
    try:
        ...
        return {"status": "success", "message": "Label berhasil dioverwrite."}
    except Exception as e:
