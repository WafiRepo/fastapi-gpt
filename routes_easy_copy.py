...
# Logging Setup
# --------------------------------------------------------
logger = logging.getLogger("easy_question_logger")
logger.setLevel(logging.INFO)
os.makedirs("./log_file", exist_ok=True)

file_handler = logging.FileHandler("./log_file/easy_question.log")
...
def interpret_graph_image(graph_path: str, language: str) -> str:
    ...
    try:
        ...
        logger.error(f"Error interpreting graph with TogetherAI: {e}")
...
def interpret_local_image(image_path: str, language: str) -> str:
    ...
    try:
        ...
        logger.error(f"Failed to interpret local image with TogetherAI: {e}")
...
def execute_python_code(code: str, language: str):
    ...
    try:
        ...
        for start, end, phase in auto_phases:
            plt.axvspan(start, end, alpha=0.3, color=phase_colors.get(phase, 'gray'), label=f'{phase} phase')
        plt.xlabel('Time (s)')
        ...
        ensure_directory("./easy_graph")
        unique_id = str(uuid.uuid4())[:8]
        saved_graph_path = f"./easy_graph/graph_{unique_id}.png"
...
# def get_latest_buffer_data(buffer_name: str, uuid_prefix: str, user_id: str):
#     connection = get_db_connection()
#     if not connection:
#         raise HTTPException(status_code=500, detail="DB connection failed.")
#     try:
...
#             ORDER BY timestamp DESC
#             LIMIT 1
#         """, (user_id, buffer_name, uuid_prefix))
#         row = cursor.fetchone()
#         if row and row["data"]:
...
def generate_easy_question(
    ...
    few_shot_examples = get_few_shot_examples(all_questions, num_examples=60)
    ...
    if "table" in selected_type:
        ...
        # --- Tambahan: Plot tabel ke gambar dan simpan lokal + base64 dengan warna phase ---
        try:
            # Label dua baris
            col_labels = [
...
- Untuk jari-jari, gunakan notasi `(r)` dalam sentimeter (cm).
...
- For radius, use the notation `(r)` in centimeter (cm).
...
- It should be simple, clear, and at an 'easy' difficulty level.
...
@routes_easy_copy.post("/easy-question")
async def easy_question(request: QuestionRequest):
